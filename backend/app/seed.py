...
DEMO_PROVIDERS = (
    ...
    ("9000000006", "Dr. Morgan Reyes", "Cardiology", "San Francisco, CA"),
...
def seed() -> None:
    with SessionLocal() as db:
        for index, (external_id, given, family, specialty, reason, document_types) in enumerate(DEMO_CASES, start=1):
            ...
            if referral is None:
                referral = Referral(patient_id=patient.id, requested_specialty=specialty, reason=reason, location_preference="San Francisco" if external_id == "SYN-1001" else None, is_synthetic=True)
                db.add(referral)
                db.flush()
            elif external_id == "SYN-1001" and referral.location_preference is None:
