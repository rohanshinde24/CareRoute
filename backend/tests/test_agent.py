...
from app.models import AgentEvent, AppointmentSlot, Coverage, Patient, PatientProcedure, Provider, ProviderSchedule, Referral, ReferralDocument, ReferralState
from app.provider_gateway import LocalProviderGateway

def build_case(db, *, document=True, coverage=True, provider=True, slot=True):
...
def test_processes_complete_referral_to_slot_selection(db):
    referral = build_case(db)
    result = run(ReferralCoordinator(db, DeterministicReferralModel()), referral)
...
def test_coordinator_uses_provider_gateway_with_workflow_correlation(db):
    referral = build_case(db)
...
def test_requests_missing_required_document(db):
    referral = build_case(db, document=False)
    result = run(ReferralCoordinator(db, DeterministicReferralModel()), referral)
    assert result.state == ReferralState.WAITING_FOR_DOCUMENTS
    assert result.missing_documents == ["clinical-note"]

def test_escalates_when_coverage_is_missing(db):
    referral = build_case(db, coverage=False)
    result = run(ReferralCoordinator(db, DeterministicReferralModel()), referral)
...
def test_conflicting_model_specialty_requires_human_review(db):
    referral = build_case(db)
    result = run(ReferralCoordinator(db, ConflictingModel()), referral)
    assert result.state == ReferralState.NEEDS_HUMAN_REVIEW
    assert result.next_action == NextAction.ESCALATE_HUMAN

def test_invalid_model_response_requires_human_review(db):
    referral = build_case(db)
    result = run(ReferralCoordinator(db, InvalidModel()), referral)
    assert result.state == ReferralState.NEEDS_HUMAN_REVIEW
    assert "Invalid structured" in result.summary

def test_ambiguous_referral_invokes_bounded_evidence_investigation(db):
...
def test_investigation_policy_rejects_premature_specialty_proposal(db):
    referral = build_case(db)
    referral.requested_specialty = "Unknown"
    db.commit()
    result = run(ReferralCoordinator(db, PrematureProposalModel()), referral)
    assert result.state == ReferralState.NEEDS_HUMAN_REVIEW
    assert "not allowed by policy" in result.summary

def test_referenced_procedure_triggers_bounded_document_investigation(db):
    referral = build_case(db)
...
def test_document_policy_rejects_premature_ungrounded_proposal(db):
    referral = build_case(db)
    referral.reason = "Holter monitoring completed last week"
    db.commit()
    result = run(ReferralCoordinator(db, UngroundedDocumentModel()), referral)
    assert result.state == ReferralState.NEEDS_HUMAN_REVIEW
    assert "not allowed by policy" in result.summary

def add_rankable_provider(db, referral, *, name="Z Preferred Cardiologist", location="San Francisco, CA", days_until_slot=2, with_slot=True):
...
def test_contextual_provider_ranking_collects_availability_then_reorders(db):
    referral = build_case(db)
    preferred = add_rankable_provider(db, referral)
...
def test_contextual_provider_ranking_abstains_without_location_match(db):
    referral = build_case(db)
    first_provider_id = db.query(Provider).one().id
    add_rankable_provider(db, referral, location="Berkeley, CA")
...
def test_contextual_provider_ranking_compares_matching_candidates(db):
    referral = build_case(db)
    later = add_rankable_provider(db, referral, name="A Later Cardiologist", days_until_slot=3)
    earlier = add_rankable_provider(db, referral, name="Z Earlier Cardiologist", days_until_slot=2)
...
def test_provider_policy_rejects_premature_proposal(db):
    referral = build_case(db)
    add_rankable_provider(db, referral)

    result = run(ReferralCoordinator(db, PrematureProviderProposalModel()), referral)
...
def test_provider_policy_rejects_out_of_candidate_proposal(db):
    referral = build_case(db)
    add_rankable_provider(db, referral)

    result = run(ReferralCoordinator(db, OutOfCandidateProviderModel()), referral)
...
def test_provider_policy_rejects_unobserved_matching_candidate(db):
    referral = build_case(db)
    add_rankable_provider(db, referral, name="A Observed Cardiologist")
    add_rankable_provider(db, referral, name="B Unobserved Cardiologist")

    result = run(ReferralCoordinator(db, UnobservedProviderModel()), referral)
...
def test_provider_policy_rejects_candidate_without_free_slots(db):
    referral = build_case(db)
    add_rankable_provider(db, referral, name="A Empty Cardiologist", location="San Francisco, CA - No availability", with_slot=False)
    add_rankable_provider(db, referral, name="B Available Cardiologist", location="San Francisco, CA - Available")

    result = run(ReferralCoordinator(db, EmptyAvailabilityProviderModel()), referral)
...
def test_provider_policy_rejects_later_availability(db):
    referral = build_case(db)
    add_rankable_provider(db, referral, name="A Later Cardiologist", days_until_slot=3)
    add_rankable_provider(db, referral, name="Z Earlier Cardiologist", days_until_slot=2)

    result = run(ReferralCoordinator(db, LaterAvailabilityProviderModel()), referral)
