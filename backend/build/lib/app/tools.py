...
def _find_providers(db: Session, request: ProviderSearchInput) -> ProviderListResult:
    records = db.scalars(select(Provider).where(Provider.specialty.ilike(request.specialty), Provider.accepting_new_patients.is_(True), Provider.is_synthetic.is_(True)).order_by(Provider.name)).all()
