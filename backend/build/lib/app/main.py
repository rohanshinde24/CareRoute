...
@app.get("/api/referrals", response_model=list[ReferralRead])
def list_referrals(db: Session = Depends(get_db)):
    return db.scalars(select(Referral).where(Referral.is_evaluation.is_(False)).order_by(Referral.created_at.desc())).all()
...
def list_slots(provider_id: uuid.UUID | None = None, specialty: str | None = Query(None, min_length=2), db: Session = Depends(get_db)):
    ...
    if specialty:
        query = query.where(Provider.specialty.ilike(f"%{specialty}%"))
    slots = db.scalars(query.order_by(AppointmentSlot.start_at)).unique().all()
    return [{"id": slot.id, "schedule_id": slot.schedule_id, "start_at": slot.start_at, "end_at": slot.end_at, "status": slot.status, "provider": slot.schedule.provider} for slot in slots]
